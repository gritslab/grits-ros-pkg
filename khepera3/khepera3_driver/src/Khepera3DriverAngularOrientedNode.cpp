#include "Khepera3DriverAngularOrientedNode.h"
...
Khepera3DriverNode::Khepera3DriverNode() {

	//Check for ID parameter
	if(!ros::param::get("khepera3/ID", myID))
		myID = 200; // default ID
	...
	ROS_INFO("Connecting to Khepera3 ID:%d at %s:%i.", myID, m_ip_address.c_str(), m_port);

	char temp[100];
	sprintf(temp,"/K%d/khepera3_send_control",myID);
	mControlSubscriber = m_node_handle.subscribe(temp, 1, &Khepera3DriverNode::send_control, this); 
...
void Khepera3DriverNode::send_control(khepera3_driver::UnicycleControlMsg req) {
...
bool Khepera3DriverNode::send_control_udp(int right_wheel_speed, int left_wheel_speed) {
...
bool Khepera3DriverNode::initialize() {
	char message[256];
...
void Khepera3DriverNode::connect() {
...
void Khepera3DriverNode::disconnect() {
	close(m_socket);
}

void Khepera3DriverNode::run() {
