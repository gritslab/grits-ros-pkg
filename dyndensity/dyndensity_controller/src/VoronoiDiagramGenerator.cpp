...
std::vector<PointVDG> VoronoiDiagramGenerator::checkAdjacency(int seed1, int seed2){
	//this function checks if two Voronoi generators seed1 and seed2 are Voronoi neighbors
	//while this can be done through Delaunay graphs, this function is implemented to avoid the numerical delicacy of Delaunay
	//returns the two vertices if they are neighbors.
	//otherwise, returns an empty vector - use this to be the criterion to tell if they were not neighbors

	std::vector<PointVDG> returnVariable;

	if (seed1 == seed2) { returnVariable.resize(0); return returnVariable;};//same as what happens at this end of this function

	//making temp copies
	std::vector<PointVDG> vertexSeed1 = voronoiOrderedList[seed1];
	std::vector<PointVDG> vertexSeed2 = voronoiOrderedList[seed2];
	
	//going to compare all the vertices in the two Voronoi cells
	//if two matching vertices are found, return 1
	//if not, return 0

	int numMatch = 0;

	for (unsigned int vertex1 = 0; vertex1 < vertexSeed1.size(); vertex1++){
		for (unsigned int vertex2 = 0; vertex2 < vertexSeed2.size(); vertex2++){
			if( vertexEqual(vertexSeed1[vertex1], vertexSeed2[vertex2]) ){
				numMatch++;
				returnVariable.push_back(vertexSeed2[vertex2]); //take note of this point
				vertexSeed2[vertex2].x = -1000; //screw up the data so that this vertex is out of the game
				vertexSeed2[vertex2].y = -1000;
			}
		}
		if (numMatch == 2) break; //happened to be Voronoi neighbors, don't need to check any more
	}

	
	if (numMatch == 2){ //if neighbors
		return returnVariable; //so natural...
	}

	//otherwise,
	returnVariable.resize(0); //force resize to an empty vector
	return returnVariable; //check the size of this thing. If it's zero, then the two agents were not neighbors.
}

void VoronoiDiagramGenerator::findOrderedList(float minX, float maxX, float minY, float maxY, float *xValues, float *yValues, int numPoints, float minDist){

	generateVoronoi(xValues,yValues,numPoints, minX,maxX,minY,maxY,minDist);
	findAllVertices(minX,maxX,minY,maxY);
	arrangeAllVertices(xValues,yValues,numPoints);

	ConvexHull* hull_generator;

	voronoiOrderedList.resize(numPoints);
	for(int seed = 0; seed < numPoints; seed++){
		hull_generator = new GrahamScanConvexHull();
		(*hull_generator)(voronoiVertexList[seed], voronoiOrderedList[seed]);
		delete hull_generator;
	}
	reset();
	
	//display what happened after ordering
/*
	std::cout << "\n\nAfter ordering:\n";
	for(int seed = 0; seed < numPoints; seed++){//loop for seeds	
		for(unsigned int vertex = 0; vertex < voronoiOrderedList[seed].size(); vertex++){ 
			printf("list[%d][%d]=(%f,%f)\t",seed,vertex,voronoiOrderedList[seed][vertex].x,voronoiOrderedList[seed][vertex].y);
		}
		printf("\n");
	}	
*/	
...
void VoronoiDiagramGenerator::arrangeAllVertices(float *xValues, float *yValues, int numPoints){

	//find to which seed all the vertices belongs.
	voronoiVertexList.clear();
	voronoiVertexList.resize(numPoints);//set the list of list of vertices to the number of seeds

	std::vector<std::vector <int> > vertexToSeedList;
	vertexToSeedList.clear();
	vertexToSeedList.resize(voronoiVertices.size());

	//building a list: for each vertex the indices of the closest seeds of same distance
	for (unsigned int vertex = 0; vertex < voronoiVertices.size(); vertex++){

		float minSquareDistance = 1000000; //ridiculously large number as a sacrifice

		for (int seed = 0; seed < numPoints; seed++){

			float temp;
			temp = (voronoiVertices[vertex].x-xValues[seed])*(voronoiVertices[vertex].x-xValues[seed]) + (voronoiVertices[vertex].y-yValues[seed])*(voronoiVertices[vertex].y-yValues[seed]);
			if(floatEqual(temp,minSquareDistance)){//if the next vertex is same distance away from the min so far
				vertexToSeedList[vertex].push_back(seed);
			}
			else if(temp < minSquareDistance){//if the next vertex is less far away from the min so far
				minSquareDistance = temp;
				vertexToSeedList[vertex].clear();
				vertexToSeedList[vertex].push_back(seed);
			}
	...
	//converting the vertex-oriented list to seed-oriented list
	for (unsigned int vertex = 0; vertex < voronoiVertices.size(); vertex++){

		for (unsigned int counter = 0; counter < vertexToSeedList[vertex].size(); counter++){
			voronoiVertexList[ vertexToSeedList[vertex][counter] ].push_back(voronoiVertices[vertex]);
		}
	}

	//display the final list
//	printf("Voronoi field %f %f %f %f\n",minX,maxX,minY,maxY);
/*
	for(int seed = 0; seed < numPoints; seed++){//loop for seeds	
		printf("Seed (%f,%f)\n",xValues[seed],yValues[seed]);
		for(unsigned int vertex = 0; vertex < voronoiVertexList[seed].size(); vertex++){ 
			printf("list[%d][%d]=(%f,%f)\t",seed,vertex,voronoiVertexList[seed][vertex].x,voronoiVertexList[seed][vertex].y);
		}
		printf("\n");
	}
*/
...
void VoronoiDiagramGenerator::findAllVertices(float minX, float maxX, float minY, float maxY){
	//finds all vertices, ands removes all duplicates.

	PointVDG tempPoint;

	if (sizeOfFinalVertices){ // if the set of voronoi vertices is not empty, put the first vertex in the vector
		tempPoint.x = finalVertices[0].x;
		tempPoint.y = finalVertices[0].y;
	}
	else{ //I guess exiting is too much...fix later
		std::cout << "Voronoi diagram is messed up! Exiting\n";
		exit(0);
	}

	voronoiVertices.clear();
	voronoiVertices.push_back(tempPoint);

	//for all the vertices, it will be put into the vector only if it's been check to make sure that it's not duplicated
	for(int i = 1; i < sizeOfFinalVertices; i++){

		//build the dot
		tempPoint.x = finalVertices[i].x;
		tempPoint.y = finalVertices[i].y;

		//checking all the previous points
		bool ignoreThis = false;
		for (unsigned int j = 0; j < voronoiVertices.size(); j++){
			if (vertexEqual(tempPoint, voronoiVertices[j])){
				ignoreThis = true;
				break;
			}
		}
	...
	//add the corners of the the field itself to the list
	std::vector<PointVDG> corners(4);
	corners[0].x = minX; corners[0].y = minY;
	corners[1].x = minX; corners[1].y = maxY;
	corners[2].x = maxX; corners[2].y = minY;
	corners[3].x = maxX; corners[3].y = maxY;

	//for all the vertices, it will be put into the vector only if it's been check to make sure that it's not duplicated
	for(unsigned int i = 0; i < corners.size(); i++){

		//build the dot
		tempPoint.x = corners[i].x;
		tempPoint.y = corners[i].y;
	...
	//display results
//	for (int i = 0; i < voronoiVertices.size(); i ++) std::cout << "("<<voronoiVertices[i].x<<", "<<voronoiVertices[i].y<<")"<<"\n";
}

bool VoronoiDiagramGenerator::generateOrderedVoronoi(float *xValues, float *yValues, int numPoints, float minX, float maxX, float minY, float maxY, float minDist){

	//generate the voronoi diagram
	this->generateVoronoi(xValues,yValues,numPoints, minX, maxX, minY, maxY, minDist);

	//get all the vertices, remove all the redundant ones.
	this->resetVerticesIterator();

//	while(this->getNextVertex(x1,y1)){
...
int VoronoiDiagramGenerator::checkBoundary(float x, float y, float minX, float maxX, float minY, float maxY){
	int boundary = 0;
	if (x == minX) boundary |= WEST;
	else if (x == maxX) boundary |= EAST;
	if (y == minX) boundary |= SOUTH;
	else if (y == maxX) boundary |= NORTH;

	return boundary;
}


bool VoronoiDiagramGenerator::vertexEqual(PointVDG v1, PointVDG v2){
	if ( floatEqual(v1.x,v2.x) && floatEqual(v1.y,v2.y) ) return true;
	else return false;
}

bool VoronoiDiagramGenerator::vertexEqual(vertexWithBoundary v1, vertexWithBoundary v2){
	if ( floatEqual(v1.coord.x,v2.coord.x) && floatEqual(v1.coord.y,v2.coord.y) ) return true;
	else return false;
}

bool VoronoiDiagramGenerator::floatEqual(float a, float b){
