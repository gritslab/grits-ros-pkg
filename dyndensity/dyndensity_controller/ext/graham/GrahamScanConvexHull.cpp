...
bool GrahamScanConvexHull::operator()(const std::vector < PointVDG >& pnt, std::vector< PointVDG >& final_hull)
{
   final_hull.clear();
   ...
   {
      point.push_back(gs_PointVDG(pnt[i].x,pnt[i].y,0.0));
   ...
   sort(++point.begin(),point.end(),GSPointVDGCompare(&anchor));
...
void GrahamScanConvexHull::graham_scan(std::vector< PointVDG >& final_hull)
{
   const std::size_t HEAD     = 0;
   const std::size_t PRE_HEAD = 1;

   std::deque<gs_PointVDG> pnt_queue;
   ...
   for(std::deque<gs_PointVDG>::iterator it = pnt_queue.begin(); it != pnt_queue.end(); it++)
   {
      final_hull.push_back(PointVDG((*it).x, (*it).y));
   }
}
...
inline int GrahamScanConvexHull::orientation(const gs_PointVDG& p1, const gs_PointVDG& p2, const gs_PointVDG& p3)
{
